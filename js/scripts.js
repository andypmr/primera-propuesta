window.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelectorAll(".video-poster").forEach((poster) => {
    poster.addEventListener("click", () => {
      const videoSrc = poster.dataset.videoSrc;
      if (!videoSrc) return;

      const iframe = document.createElement("iframe");
      iframe.src = videoSrc;
      iframe.title = poster.getAttribute("aria-label") || "Video de Venzo en YouTube";
  ...
  featureCards.forEach((card) => {
    card.addEventListener("click", () => {
      ...
      featureCards.forEach((item) => item.classList.remove("is-open"));
      card.classList.toggle("is-open", shouldOpen);
      if (!shouldOpen) card.blur();
    });
  });
